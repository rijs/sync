...
var emitterify$3 = function emitterify(body, hooks) {
  body = body || {};
  hooks = hooks || {};
  ...
  function remove(li, cb) {
    ...
    while (~--i) 
      { if (cb == li[i] || cb == li[i].fn || !cb)
        { (hooks.off || noop$1)(li.splice(i, 1)[0]); } }
  }
...
var key$3 = function key(k, v){ 
  var set = arguments.length > 1
    , keys = is_1$2.fn(k) ? [] : str$4(k).split('.').filter(Boolean)
    , root = keys.shift();
...
: is_1$2.obj(name)         ? server.send(name)
...
var cache = function (ripple, n, k) { return function (change) {
  ...
  if (is_1$2.def(k)) { change.key = k + "." + (str$4(change.key)); }
...
var subscribe = function (ripple) { return function (name, k) {
  ...
  if (is_1$2.arr(k)) { return merge(k.map(function (k) { return ripple.subscribe(name, k); }))
    .map(function (d) { return key$3(k)(ripple(name)); }) }
  var output = emitterify$3().on('subscription');
